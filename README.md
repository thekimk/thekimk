...
| 정상적인 컴퓨터라면 수업 진행에 이슈는 없지만, 컴퓨팅 환경이 이상하거나 다루는데 익숙하지 않으면 `예상하진 못한 어려움 발생 가능` <br> (ex. `강의자료 다운받고 파일을 찾지 못한다` or `이메일 첨부파일 업로드 안된다` or `강의자료 실행하니 영화가 실행된다` or `인터넷이 안켜진다` 등등) |
