...
> - **Why?:**
>
> : 여러분들을 `과대평가/괴롭히기` 목적이 아니라, 세상은 `너무 복잡하고 어렵고 빠르게 변하는데` 언제나 우린 `상대적으로 과소한 존재`이고 과거의 지식으로 미래에 대응하기 어렵기 때문에, 이를 `미리 경험`해서 미래에는 `좌절하거나 포기하는 것들을 줄이기 위함`    
>
> : 본 강의들도 미래의 여러분들에게 `쓸모없을 수` 있지만, 남는 것은 `기회가 왔을때 놓치지 않기 위해 배우지 않았어도 스스로 학습하고 창의적으로 문제를 해결하는 능력`을 함양하는 것이 유일한 방법이기 때문
